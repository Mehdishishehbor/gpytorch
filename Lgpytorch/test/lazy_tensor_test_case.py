...
import Lgpytorch
from Lgpytorch.settings import linalg_dtypes
from Lgpytorch.utils.errors import CachingError
from Lgpytorch.utils.memoize import get_from_cache
...
class RectangularLazyTensorTestCase(BaseTestCase):
    ...
    def test_getitem_tensor_index(self):
        ...
        if lazy_tensor.ndimension() == 2:
            ...
            res, actual = Lgpytorch.delazify(lazy_tensor[index]), evaluated[index]
            self.assertAllClose(res, actual)
    ...
    def test_quad_form_derivative(self):
        ...
        deriv_auto = Lgpytorch.lazy.LazyTensor._quad_form_derivative(lazy_tensor_clone, left_vecs, right_vecs)
...
class LazyTensorTestCase(RectangularLazyTensorTestCase):
        ...
        if not self.__class__.skip_slq_tests:
            ...
            with patch("gpytorch.utils.linear_cg", new=_wrapped_cg) as linear_cg_mock:
                with Lgpytorch.settings.num_trace_samples(256), Lgpytorch.settings.max_cholesky_size(
                    ...
                    with Lgpytorch.settings.min_preconditioning_size(4), Lgpytorch.settings.max_preconditioner_size(2):
    ...
    def test_prod(self):
        with Lgpytorch.settings.fast_computations(covar_root_decomposition=False):
            lazy_tensor = self.create_lazy_tensor()
            evaluated = self.evaluate_lazy_tensor(lazy_tensor)
    ...
    def test_root_decomposition(self, cholesky=False):
        _wrapped_lanczos = MagicMock(wraps=Lgpytorch.utils.lanczos.lanczos_tridiag)
        with patch("gpytorch.utils.lanczos.lanczos_tridiag", new=_wrapped_lanczos) as lanczos_mock:
            ...
            with Lgpytorch.settings.max_cholesky_size(math.inf if cholesky else 0):
    ...
    def test_diagonalization(self, symeig=False):
        ...
        with patch("gpytorch.utils.lanczos.lanczos_tridiag", new=_wrapped_lanczos) as lanczos_mock:
            lazy_tensor = self.create_lazy_tensor()
            test_mat = torch.randn(*lazy_tensor.batch_shape, lazy_tensor.size(-1), 5)
            with Lgpytorch.settings.max_cholesky_size(math.inf if symeig else 0):
    ...
    def _test_triangular_lazy_tensor_inv_quad_logdet(self):
        ...
        with Lgpytorch.settings.max_cholesky_size(math.inf):
            lazy_tensor = self.create_lazy_tensor()
            rootdecomp = lazy_tensor.root_decomposition()

            if isinstance(rootdecomp, Lgpytorch.lazy.CholLazyTensor):
                chol = lazy_tensor.root_decomposition().root.clone()
                Lgpytorch.utils.memoize.clear_cache_hook(lazy_tensor)
                Lgpytorch.utils.memoize.add_to_cache(
                    lazy_tensor,
                    "root_decomposition",
                    Lgpytorch.lazy.RootLazyTensor(chol),
                )
    ...
    def test_sqrt_inv_matmul(self):
        ...
        with Lgpytorch.settings.max_cg_iterations(200):
