...
from Lgpytorch.likelihoods import GaussianLikelihood, FixedNoiseGaussianLikelihood
...
class TestWhiteNoiseGPRegression(unittest.TestCase):
    ...
    def test_posterior_latent_gp_and_likelihood_with_optimization(self, cuda=False, cg=False):
        ...
        optimizer.n_iter = 0
        with Lgpytorch.settings.debug(False), Lgpytorch.settings.max_cholesky_size(0 if cg else 1e10):
            for _ in range(75):
                optimizer.zero_grad()
