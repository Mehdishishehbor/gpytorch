...
from Lgpytorch.utils.warnings import OldVersionWarning
...
class TestSVGPRegression(BaseTestCase, unittest.TestCase):
    ...
    def test_loading_old_model(self):
        ...
        model = SVGPRegressionModel(torch.linspace(0, 1, 25), Lgpytorch.variational.CholeskyVariationalDistribution)
    ...
    def test_predictive_ll_regression_error_delta(self):
        return self.test_regression_error(
            mll_cls=Lgpytorch.mlls.PredictiveLogLikelihood,
            distribution_cls=Lgpytorch.variational.DeltaVariationalDistribution,
        )

    def test_robust_regression_error(self):
        return self.test_regression_error(mll_cls=Lgpytorch.mlls.GammaRobustVariationalELBO)
